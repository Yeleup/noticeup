...
class ModelCatalogManufacturer extends Model {
    ...
    public function copyManufacturer($manufacturer_id, $store) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "manufacturer m WHERE m.manufacturer_id = '" . (int)$manufacturer_id . "'");

        if ($query->num_rows) {
            $data = $query->row;

            $data['manufacturer_store'] = $store;
            $data['manufacturer_seo_url'] = $this->getManufacturerSeoUrls($manufacturer_id);
