<?php
class ControllerExtensionStorePaymentCheque extends Controller {
	private $error = array();
    private $store_id = 0;
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('payment_cheque', $this->request->post, $this->store_id);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/store_payment/cheque', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/store_payment/cheque', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=store_payment', true);

        $payment_cheque = $this->model_setting_setting->getSetting('payment_cheque', $this->store_id);

		if (isset($this->request->post['payment_cheque_payable'])) {
			$data['payment_cheque_payable'] = $this->request->post['payment_cheque_payable'];
		} elseif ($payment_cheque) {
            $data['payment_cheque_payable'] = $payment_cheque['payment_cheque_payable'];
        } else {
		...
		if (isset($this->request->post['payment_cheque_total'])) {
            ...
            $data['payment_cheque_total'] = $payment_cheque['payment_cheque_total'];
        } else {
		...
		if (isset($this->request->post['payment_cheque_order_status_id'])) {
            ...
            $data['payment_cheque_order_status_id'] = $payment_cheque['payment_cheque_order_status_id'];
        } else {
		...
		if (isset($this->request->post['payment_cheque_geo_zone_id'])) {
            ...
            $data['payment_cheque_geo_zone_id'] = $payment_cheque['payment_cheque_geo_zone_id'];
        } else {
		...
		if (isset($this->request->post['payment_cheque_status'])) {
            ...
            $data['payment_cheque_status'] = $payment_cheque['payment_cheque_status'];
        } else {
		...
		if (isset($this->request->post['payment_cheque_sort_order'])) {
            ...
            $data['payment_cheque_sort_order'] = $payment_cheque['payment_cheque_sort_order'];
        } else {
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/store_payment/cheque')) {
