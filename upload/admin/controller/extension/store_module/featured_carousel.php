<?php
class ControllerExtensionStoreModuleFeaturedCarousel extends Controller {
	private $error = array();
    private $store_id = 0;
	...
	public function index() {
		$this->load->language('extension/module/featured_carousel');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_module->editStoreModule($this->request->get['module_id'], $this->store_id, 'featured_carousel', $this->request->post);
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/store_module/featured_carousel', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/store_module/featured_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/store_module/featured_carousel', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/store_module/featured_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->response->setOutput($this->load->view('extension/store_module/featured_carousel', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/store_module/featured_carousel')) {
