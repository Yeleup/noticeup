<?php
class ControllerExtensionStoreModuleBestSellerCarousel extends Controller {
	private $error = array();
    private $module_id = 0;
    ...
    public function __construct($registry)
    {
        ...
        $this->module_id = $this->request->get['module_id'];

        $userac = $this->registry->get('user');
        $this->user_store_id = $userac->store_id;
    }

	public function index() {
		$this->load->language('extension/module/bestseller_carousel');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_module->editStoreModule($this->request->get['module_id'], $this->user_store_id, 'bestseller_carousel', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=store_module', true)
		);

		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/store_module/bestseller_carousel', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/store_module/bestseller_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/store_module/bestseller_carousel', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/store_module/bestseller_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);

		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$module_info = $this->model_setting_module->getStoreModule($this->request->get['module_id'], $this->user_store_id);
		}
		...
		$this->response->setOutput($this->load->view('extension/store_module/bestseller_carousel', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/store_module/bestseller_carousel')) {
