...
class ModelStoreProduct extends Model {
	...
	public function editProduct($product_id, $data) {
        ...
        $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_store SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$this->store_id . "'");
	...
	public function getProducts($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2s.store_id = '". (int)$this->store_id ."'";
	...
	public function getTotalProducts($data = array()) {
		...
		$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2s.store_id = '" . (int)$this->store_id . "'";
    ...
    public function checkProductForStore($product_id){
        $sql = "SELECT count(product_id) as total FROM " . DB_PREFIX . "product_to_store p2s WHERE p2s.product_id='". (int)$product_id ."' AND p2s.store_id = ".(int)$this->store_id;
