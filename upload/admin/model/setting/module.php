...
class ModelSettingModule extends Model {
    ...
    public function editStoreModule($module_id, $store_id, $code, $data) {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "store_module` SET `module_id` = '" . (int)$module_id . "', `store_id` = '" . (int)$store_id . "', `name` = '" . $this->db->escape($data['name']) . "', `code` = '" . $this->db->escape($code) . "', `setting` = '" . $this->db->escape(json_encode($data)) . "' ON DUPLICATE KEY UPDATE `name` = '" . $this->db->escape($data['name']) . "', `setting` = '" . $this->db->escape(json_encode($data)) . "'");
    }

    public function getStoreModule($module_id, $store_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "store_module` WHERE `module_id` = '" . (int)$module_id . "' AND `store_id` = '" . (int)$store_id . "'");
